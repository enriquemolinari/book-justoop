...
public class Cinema implements CinemaSystem {
    ...
    public MovieInfo movie(Long id) {
        return emf.callInTransaction(em -> {
            try {
                return movieWithActorsById(em, id);
    ...
    private MovieInfo movieWithActorsById(EntityManager em, Long id) {
        return em
    ...
    public DetailedShowInfo reserve(Long userId, Long showTimeId,
        ...
        return emf.callInTransaction(em -> {
            ShowTime showTime = showTimeBy(em, showTimeId);
    ...
    public Long registerUser(String name, String surname, String email,
        ...
        return inTxWithRetriesOnConflict((em) -> {
            checkUserNameAlreadyExists(em, userName);
    ...
    private void checkUserIsRatingSameMovieTwice(EntityManager em, Long userId, Long movieId) {
        var q = em.createQuery(
    ...
    private Theater theatreBy(EntityManager em, Long theatreId) {
        return findByIdOrThrows(em, Theater.class, theatreId, THEATER_ID_DOES_NOT_EXISTS);
    }

    private Movie movieBy(EntityManager em, Long movieId) {
        return findByIdOrThrows(em, Movie.class, movieId, MOVIE_ID_DOES_NOT_EXISTS);
    }

    private User userBy(EntityManager em, Long userId) {
        return findByIdOrThrows(em, User.class, userId, USER_ID_NOT_EXISTS);
    }

    private ShowTime showTimeBy(EntityManager em, Long id) {
        return findByIdOrThrows(em, ShowTime.class, id, SHOW_TIME_ID_NOT_EXISTS);
    }
    ...
    public void changePassword(Long userId, String currentPassword,
        ...
        emf.callInTransaction(em -> {
            userBy(em, userId).changePassword(currentPassword, newPassword1,
                    newPassword2);
